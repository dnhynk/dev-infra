...
사용자는 이 터미널을 보고 있지 않다. 이동 중에 Slack만 본다. 터미널 프롬프트로 물으면
그 질문은 아무에게도 도달하지 않고 Run 전체가 그 자리에서 멈춘다.
...
- **저확신은 인상이 아니라 관측으로 판정한다.** 완료 보고가 결론 대신 선택지를 나열한다 / 유보·조건부 표현으로만 답한다 / 주장을 뒷받침하는 실행·관측 결과가 없다 / 보고 안의 서술이 서로 상충한다 / 검증 가능한 완료 기준을 만족했다고 말하지 못한다. 어느 신호도 관측되지 않으면 결과가 미덥지 않아도 escalate하지 않는다.
- **coordinator가 확정 스펙·코드·live 상태로 직접 판정할 수 있으면 escalate하지 않고 그것으로 닫는다.**
- **재dispatch에 이전 worker의 결론과 미해결 지점을 함께 넘긴다.** 같은 출발점에서 다시 시작하면 같은 답이 나온다.
- 배치가 바뀌므로 terminal을 재사용할 수 없다 — `worker-release` 후 새 agent terminal을 만든다.
- 11행 배치로도 확답에 이르지 못하면 다시 escalate하지 않고 사용자에게 올린다.

7b행은 11행과 목적이 다르다. 11행은 실패가 관측된 뒤의 **사후** escalation이고, 7b행은
실패가 조용해서 사후 신호가 오지 않는 영역의 **사전** 위험 트리거다. 일반 코드 리뷰(7행)를
7b로 올리지 않는다 — 판정 기준의 신호가 관측될 때만 적용한다.
