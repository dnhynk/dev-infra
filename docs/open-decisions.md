...
| ID | 결정 | 필요한 시점 | 상태 |
|---|---|---|---|
| OD-020 | Run↔repo cardinality와 Run↔worktree↔coordinator session identity | S0 전 | DECIDED |
...
ID: OD-020
상태: DECIDED
결정: Orca Run row를 권위로 삼는다. `run-list`가 반환하는 `coordinator_handle`·
      `coordinator_pane_key`·`consumer_generation`이 권위 있는 identity다.
      coordinator 세션의 `ORCA_TERMINAL_HANDLE`·`ORCA_PANE_KEY`·`ORCA_WORKTREE_ID` 같은
      환경변수는 보조 단서로만 쓴다. repository는 설정 파일의 수동 등록(OD-068)과 연결한다.
      live/stale Run은 `consumer_generation`으로 구분한다.
근거:
  - 2026-08-22 AB-0 환경 관측에서 환경변수와 Run row 필드가 같은 값이고 `ORCA_WORKTREE_ID`의
...
영향 문서/파일: docs/contracts/observation-and-correlation.md §5,
                docs/architecture/orca-slack-bridge.md §2,
                docs/specs/orchestration-bootstrap-and-continuity.md §9·§11,
                docs/roadmap.md §7, docs/traceability.md, 향후 D1 discovery/collector
검증 방법: `run-list` snapshot에서 handle·pane key·generation을 읽고 `run-use` 인수 전후 generation이
           증가하며 이전 handle binding이 stale로 분류되는 fixture로 검증한다. 설정에 수동 등록한
           repository만 Run과 연결되고 환경변수 불일치가 Run row를 덮어쓰지 않는지도 확인한다.
