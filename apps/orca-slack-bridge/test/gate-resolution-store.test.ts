...
describe('strict persisted Gate schema', () => {
  it('fresh v12와 v7→v8→v9→v10→v11→v12가 같은 additive resolution tables를 만든다', () => {
    ...
    expect(migrated.prepare('SELECT version FROM schema_version WHERE id = 1').get()).toEqual({ version: 16 });
